...
def is_nondiff(value: Any) -> bool:
    """Returns True if the node is a non-differentiable node and False otherwise.
    ...
    Note:
        ...
        - Differentiable types are: float, complex number, or a numpy array of floats or
        complex numbers.
...
def tree_mask(tree: T, mask: MaskType = is_nondiff, *, is_leaf: IsLeafType = None):
    """Mask tree leaves according to `mask`.
    
    Masked leaves are excluded from `jax.tree_util` functions. defaults to 
    masking non-inexact nodes.
